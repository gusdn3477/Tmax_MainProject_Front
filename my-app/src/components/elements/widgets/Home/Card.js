  ...
  if (data.closed === 'T')
    return (
      <div className="card" style={{ width: "60rem", margin: "13px"}}>
        <div className="card-inverse-info card-inverse-info-position ">마감된 공고입니다.</div>
        <div className="card-body">
          ...
          {data.applyStart && data.applyEnd ?
            <p className="card-text" style={{ color:"red", fontWeight:"bold"}}>지원기간 : {(data.applyStart).substring(0, 10)} ~ {(data.applyEnd).substring(0, 10)}</p> : ""
          }
          <Link to={`/jobs/${data.jobsNo}`}>
            <button type="button" className="btn btn-primary" style={{ marginTop:"0.5rem"}}>공고보기</button>
          </Link>
        </div>
      </div>
    );
